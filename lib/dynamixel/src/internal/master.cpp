...
Master::setPortProtocolVersion(float version)
{
  ...
  if(version == 2.0){
    ...
    last_lib_err_ = DXL_LIB_ERROR_INVALID_PROTOCOL_VERSION;
    return false;
  }
...
Master::ping(uint8_t id, uint8_t *p_recv_id_array, uint8_t recv_array_capacity, uint32_t timeout_ms)
{
  ...
  if(txInstPacket(id, DXL_INST_PING, nullptr, 0) == true){
    ...
    if(id != DXL_BROADCAST_ID){
      ...
      timeout_ms = 3*253;
      uint64_t pre_time_ms = detail::currentMillis();
      while(ret_id_cnt < recv_array_capacity)
      {
        ...
        if (detail::currentMillis()-pre_time_ms >= timeout_ms) {
...
Master::writeNoResp(uint8_t id, uint16_t addr, const uint8_t *p_data, uint16_t data_length)
{
  ...
  if(p_data == nullptr || p_port_ == nullptr){
    ...
    err = DXL_LIB_ERROR_INVALID_DATA_LENGTH;
  }
  if(err != DXL_LIB_OK){
    last_lib_err_ = err;
    return false;
...
// Function to Return a string for the requested err_code from the error codes defined in DXLLibErrorCode enum:

const char*
Master::getLibErrCodeStr(DXLLibErrorCode_t err_code)
{
  switch(err_code){
    case DXL_LIB_OK:
      return "DXL_LIB_OK";
    ...
    case DXL_LIB_ERROR_TIMEOUT: 
      return "DXL_LIB_ERROR_TIMEOUT";
    case DXL_LIB_ERROR_INVALID_ID:
      return "DXL_LIB_ERROR_INVALID_ID";
    ...
    case DXL_LIB_ERROR_INVALID_ADDR:
      return "DXL_LIB_ERROR_INVALID_ADDR";
    case DXL_LIB_ERROR_ADDR_LENGTH:
      return "DXL_LIB_ERROR_ADDR_LENGTH";
    case DXL_LIB_ERROR_BUFFER_OVERFLOW:
      return "DXL_LIB_ERROR_BUFFER_OVERFLOW";
    case DXL_LIB_ERROR_PORT_NOT_OPEN:
      return "DXL_LIB_ERROR_PORT_NOT_OPEN";
    case DXL_LIB_ERROR_WRONG_PACKET:
      return "DXL_LIB_ERROR_WRONG_PACKET";
    case DXL_LIB_ERROR_CHECK_SUM:
      return "DXL_LIB_ERROR_CHECK_SUM";
    case DXL_LIB_ERROR_CRC: 
      return "DXL_LIB_ERROR_CRC";
    case DXL_LIB_ERROR_INVALID_DATA_LENGTH: 
      return "DXL_LIB_ERROR_INVALID_DATA_LENGTH";
    case DXL_LIB_ERROR_MEMORY_ALLOCATION_FAIL:
      return "DXL_LIB_ERROR_MEMORY_ALLOCATION_FAIL";
    case DXL_LIB_ERROR_INVALID_PROTOCOL_VERSION:
      return "DXL_LIB_ERROR_INVALID_PROTOCOL_VERSION";
    ...
    case DXL_LIB_ERROR_PORT_WRITE:
      return "DXL_LIB_ERROR_PORT_WRITE";
    default:
    {
          // create a string with the error code:
          static char err_code_str_[64];
          snprintf(err_code_str_, sizeof(err_code_str_), "Unknown code: %d", int(err_code));
          return err_code_str_;
    }
  }
}
