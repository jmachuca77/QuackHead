...
void Audio::playChunk() {
    if (m_sampleFilter != NULL) {
        m_sampleFilter(getBitsPerSample(), m_channels, &m_outBuff[m_curSample], m_validSamples);
    }
...
void Audio::loop() {
    ...
    if(m_playlistFormat != FORMAT_M3U8) { // normal process
        switch(getDatamode()) {
            case AUDIO_NULLSTREAM: processNullStream(); break;
...
//---------------------------------------------------------------------------------------------------------------------
void Audio::processNullStream() {
    if (m_f_running && getDatamode() == AUDIO_NULLSTREAM) {
        // memset(m_outBuff,   0, m_outbuffSize);     //Clear OutputBuffer (signed)
        if(getBitsPerSample() > 8) memset(m_outBuff,   0, m_outbuffSize);     //Clear OutputBuffer (signed)
        else                       memset(m_outBuff, 128, m_outbuffSize);     //Clear OutputBuffer (unsigned, PCM 8u)

        m_validSamples = 256;
        m_curSample = 0;
        //computeAudioTime(m_validSamples * 2);

        int bd = m_validSamples * 2;
        {
...
#else
    esp_err_t err = i2s_write((i2s_port_t)m_i2s_num, (const char*)&s32, sizeof(uint32_t), &m_i2s_bytesWritten, 1000); // no wait
#endif
    if(err != ESP_OK) {
