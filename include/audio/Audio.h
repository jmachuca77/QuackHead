...
public:
    ...
    inline void setSampleFilter(AudioSampleFilter filter) {
        m_sampleFilter = filter;
    }

    inline bool     isNullStream() { return m_datamode == AUDIO_NULLSTREAM; }
    inline bool     isLocalFile() { return m_datamode == AUDIO_LOCALFILE; }
    inline bool     isWebStream() { return !isNullStream() && !isLocalFile() && m_datamode != AUDIO_NONE; }

    virtual void audio_eof_mp3(const char*) {}
...
private:
    ...
    enum : int { AUDIO_NONE, HTTP_RESPONSE_HEADER, AUDIO_DATA, AUDIO_LOCALFILE,
                 AUDIO_PLAYLISTINIT, AUDIO_PLAYLISTHEADER,  AUDIO_PLAYLISTDATA, AUDIO_NULLSTREAM};
...
#pragma GCC diagnostic pop
    ...
    AudioSampleFilter m_sampleFilter = NULL;
};
